...
public class You {
	...
	public VoiceCommand getNextStep(Clark clark, CityMap cityMap) {
		...
		// TODO: Implement algorithm to return command to Clark to capture all the
		// targets on the city map provided

		// <<SOLUTION START>>

		BrakeDistance brakeDistance = new BrakeDistance();

		double brakeDistanceX = brakeDistance.calculateBrakeDistance(Math.abs(clark.getHorizontal()));
		double brakeDistanceY = brakeDistance.calculateBrakeDistance(Math.abs(clark.getVertical()));
		...
		} else if ((targetToCapture.x > clark.getPosition().x) && (diffX > brakeDistanceX)) {
			voiceCommand = new VoiceCommand(Direction.EAST, horizontalSpeedLevel);
		} else if ((targetToCapture.y > clark.getPosition().y) && (diffY > brakeDistanceY)) {
